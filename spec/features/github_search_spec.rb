...
RSpec.describe "Searching Github", type: :feature do
  ...
  it "displays the right data" do
    stubbed_request = stub_request(:get, "https://api.github.com/search/repositories").with(query: hash_including({q: "tetris+language:assembly", sort: "stars", order: "desc"})).to_return(body: repository_response)
