...
RSpec.describe "Searching Github", type: :feature do
  ...
  it "displays the right data" do
    stubbed_request = stub_request(:get, "https://api.github.com/search/repositories").with(query: hash_including({q: "dtrupenn/Tetris"})).to_return(body: repository_response)

    visit '/search'
    fill_in 'query', with: "dtrupenn/Tetris"
